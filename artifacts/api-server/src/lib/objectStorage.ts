...
const SIDECAR_ENDPOINT =
  process.env["OBJECT_STORAGE_SIDECAR_ENDPOINT"] ?? "http://127.0.0.1:1106";

export const objectStorageClient = new Storage({
  credentials: {
    audience: process.env["OBJECT_STORAGE_AUDIENCE"] ?? "object-storage",
...
export class ObjectAccessForbiddenError extends Error {
  constructor() {
    super("Object access forbidden");
    this.name = "ObjectAccessForbiddenError";
    Object.setPrototypeOf(this, ObjectAccessForbiddenError.prototype);
  }
}
...
export class ObjectStorageService {
  ...
  getPrivateObjectDir(): string {
    ...
    if (!dir) {
      throw new Error(
        "PRIVATE_OBJECT_DIR not set. Create a bucket and set " +
  ...
  async trySetObjectEntityAclPolicy(
    ...
    // Prevent ownership takeover: if the object already has an owner that isn't
    // the caller, refuse to re-own it or change its visibility.
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to sign object URL, errorcode: ${response.status}, ` +
        `make sure the object storage sidecar is reachable`
    );
  }
